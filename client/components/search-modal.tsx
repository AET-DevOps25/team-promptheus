...
export function SearchModal({ isOpen, onCloseAction, usercode }: SearchModalProps) {
  ...
  const getTypeIcon = (type: string) => {
    const Icon = contributionTypeIcons[type as keyof typeof contributionTypeIcons];
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      className="mb-4"
                      onChange={(e) =>
                        handleFilterChange("contributionType", e.target.value || undefined)
                      }
                    ...
                    <Input
                      className="mb-4"
                      onChange={(e) => handleFilterChange("week", e.target.value || undefined)}
                ...
                <div className="flex justify-between items-center">
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {filters.user && <Badge variant="secondary">User filtered</Badge>}
                    {(filters.dateFrom || filters.dateTo) && (
          ...
          <div className="space-y-4">
            {isLoading ? (
                ...
                <div className="flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">
                    ...
                    {searchResponse?.processingTimeMs && (
                      <span className="ml-2">({searchResponse.processingTimeMs}ms)</span>
                    )}
                  </p>
                  <Button className="text-xs" onClick={clearFilters} size="sm" variant="ghost">
                    Clear filters
                  </Button>
                </div>
                {results.map((result: SearchResultItem, index: number) => (
                  <Card className="hover:bg-muted/50 transition-colors" key={result.id || index}>
                    <CardContent className="p-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            {result.type && (
                              ...
                              >
                                ...
                                <span className="ml-1">
                                  {contributionTypeLabels[result.type] || result.type}
                                </span>
                              </Badge>
                            )}
                            {result.repository && (
                          ...
                          <div className="flex items-center gap-4 text-xs text-muted-foreground">
                            ...
                            {result.created_at_timestamp && !result.date && (
                              <span>
                                {new Date(result.created_at_timestamp * 1000).toLocaleDateString()}
                              </span>
                            )}
                          </div>
                        </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Search commits, pull requests, issues, and comments with AI-powered semantic
                  understanding
                </p>
              </div>
            )}
