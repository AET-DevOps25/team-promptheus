...
export function useGitRepoInformation(usercode: string, enabled = true) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<GitRepoInformation>(`/api/repositories/${usercode}`);
...
export function useCreateFromPAT() {
  return useMutation({
    mutationFn: async (patData: PATConstruct) => {
      const response = await apiClient.post<LinkConstruct>("/api/repositories/PAT", patData);
      return response.data;
    },
  });
}
