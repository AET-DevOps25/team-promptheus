...
class ContributionsIngestionService:
    ...
    def __init__(self, meilisearch_service: Optional[MeilisearchService] = None, github_token: Optional[str] = None, summary_service=None):
    ...
    async def _process_ingestion_task(self, task_id: str, request: ContributionsIngestRequest, summary_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            if task_id in self.ingest_tasks:
                ...
                self.ingest_tasks[task_id].failed_count = len([m for m in request.contributions if m.selected]) - len(contributions)
            ...
            if task_id in self.ingest_tasks:
                ...
                if self.ingest_tasks[task_id].started_at:
                    processing_time = self.ingest_tasks[task_id].completed_at - self.ingest_tasks[task_id].started_at
                    self.ingest_tasks[task_id].processing_time_ms = int(processing_time.total_seconds() * 1000)
            
            logger.info("Full task completed successfully", 
                       task_id=task_id, 
                       user=request.user, 
                       week=request.week,
                       status="done")
    ...
    async def ingest_contributions(self, request: ContributionsIngestRequest) -> ContributionsIngestResponse:
        ...
        try:
            ...
            # Process the fetched contributions
            ingested_count, failed_count, embedding_job_id = await self._ingest_contributions_content(
    ...
    async def _ingest_contributions_content(self, user: str, week: str, contributions: List[GitHubContribution]) -> tuple[int, int, str]:
