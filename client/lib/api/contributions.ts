...
/**
 * Hook to update multiple contributions
 */
export function useUpdateContributions() {
	...
	>({
		...
		onMutate: async (newContributions) => {
			...
			const previousContributions = queryClient.getQueriesData({
				queryKey: CONTRIBUTION_KEYS.all,
			});

			// Optimistically update to the new value
			queryClient.setQueriesData(
				{ queryKey: CONTRIBUTION_KEYS.all },
				(old: PageContributionDto | undefined) => {
					if (!old?.content) return old;

					// Create a map for quick lookup of updated contributions
					const updateMap = new Map(
						newContributions.map((contrib) => [contrib.id, contrib]),
					);
